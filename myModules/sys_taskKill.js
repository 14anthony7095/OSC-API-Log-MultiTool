...
//	--	On Load	--
console.log(`${loglv.info}${selflog} Loaded -> ${loglv.bool(vrckiller)}${vrckiller}${loglv.reset}`)
killprep()

//	--	Functions	--
async function killprep() {
	return new Promise((resolve, reject) => {
		console.log(`${loglv.info}${selflog} Looking up VRChat's process ID`)
		find('name', "VRChat.exe", true).then((list, err) => {
			if (err) console.error(err);
			// if (list.length > 1) {
			// 	console.log(`${loglv.info}${selflog} VRChat's process ID is ${list[0].pid}`)
			// 	vrchatprocessidArr
...
function killvrc(delay) {
	...
	setTimeout(() => {
		...
		console.log(`${loglv.info}${selflog} Resetting process ID target to NULL`)
...
oscEmitter.on('osc', (address, value) => {
	...
	if (vrckiller == true) {
		// exitGame
		// if (address == '/avatar/parameters/VRCEmote' && value == 96) {			killvrc(1)		}
	}
});