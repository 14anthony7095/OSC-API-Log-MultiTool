	...
	if( cmd == 'heart' && args[0] == 'random' ){
		if( args[1] == 'true' ){
			hrmRandomizer = setInterval(()=>{
				...
				console.log(`${loglv.info}${selflog} BPM: ${randrate}` )
				...
				console.log(`${loglv.info}${selflog} OSC Data: ${parseFloat( randrate / 255 - 1 )}` )
			},2000)
		}
...
function start() {
	if( wsopenned == false ){
		...
		console.log(`${loglv.info}${selflog} WS is already running, continuing to use last session.`)
	}
}
...
function stop() {
	isActive = false
	console.log(`${loglv.info}${selflog} Stopping, but will continue to run in background until crashes`)
}
var lastbpm = 0
var lastbpmUpdate = 0
var reconDelay;
function openWebSocket() {
	...
	ws.on('open', e =>{
		wsopenned = true
		console.log(`${loglv.info}${selflog} WebSocket Connection Open to PULSOID`)
		ws.on('message', ev => {
			...
			if( lastbpm != data.data.heartRate ){
				...
				if(Date.now() > lastbpmUpdate+60_000){
					...
					console.log(`${loglv.info}${selflog} BPM: ${data.data.heartRate}` )
				}
