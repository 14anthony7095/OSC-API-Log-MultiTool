...
async function fsReadandClose() {
	var fileHandler;
	var output = []
	try {
		fileHandler = await fsp.open('./datasets/worldSeen.txt')
		var fileRead = await fileHandler.readFile('utf8')
		var arrayByLine = fileRead.split('\r\n')
		console.log(`${loglv.info}${selflogL} Worlds-Visited contains ${arrayByLine.length} entries.`)
		output = new Set(arrayByLine)
	} catch (error) {
		console.log(`${loglv.warn}${selflogL}`, error)
	} finally { if (fileHandler) { await fileHandler.close(); worldsSeenDB = output } }
}
fsReadandClose()
...
-	twitch
	...
	if (cmd == 'hoppers') {
		var string = ''
		worldHoppers['users'].sort((a, b) => { return b.playtime - a.playtime }).forEach((a) => {
		...
		worldHoppers['worlds'].forEach((w, index) => { string += `${index == 0 ? `\n` : ''}\n${w}` })
		console.log(string)
	}
	if (cmd == 'clearhoppers') {
		console.log(`${loglv.hey}[WorldHop] Cleared ${worldHoppers['users'].length} users from tracker`)
		worldHoppers = { 'worlds': [], 'users': [] }
	}
})
...
function processLogLine(line) {
	...
	if (lineC.startsWith('[ToNBR]') && InstanceHistory[0].worldID == 'wrld_af78055a-60f2-4ed6-a1d1-6e486e1e53bf') {
		...
		for (const item in tonbrConsts.tonmasks) {
			var match = ToNBRLog.match(tonbrConsts.tonmasks[item].regex)
			if (match) {
				var toncontext = tonbrConsts.tonmasks[item].context
				switch (toncontext) {
					case 'Perk':
						/*
						// Possible Perks:
							'None', 'Sinner', 'Runner', 'Electrician', 'Chef',
					...
					case 'RoundType':
						// Classic , Chaos , Custom
						tonbrRoundData['roundtype'] = match[1];

						console.log(`${loglv.info}${selflogL} [ToNBR] Round-Type: ${tonbrRoundData['roundtype']}`);
						break;

					case 'RoundEnd':
						tonbrRoundData['roundactive'] = false;
						tonbrRoundData['terrorlist'] = [];
						tonbrRoundData['alive'] = true;
					...
					case 'Wave':
						// 0 - 10
						tonbrRoundData['wave'] = parseInt(match[1]);

						console.log(`${loglv.info}${selflogL} [ToNBR] Wave: ${tonbrRoundData['wave']}`);
						break;

					case 'FoundItem':
						tonbrRoundData['item'] = match[1];

						console.log(`${loglv.info}${selflogL} [ToNBR] Item-Found: ${tonbrRoundData['item']}`);
						break;
					...
					case 'Location':
						tonbrRoundData['location'] = match[1];

						console.log(`${loglv.info}${selflogL} [ToNBR] Location: ${tonbrRoundData['location']}`);
						break;

					case 'TerrorStart':
						tonbrRoundData['terrorlist'] = ToNBRLog.match(/[^,]+?(?=\s*\(\d+\))/g).filter((f, index) => index % 2 == 0).map(name => name.replace(/ \{\d+\}/, '').trim());

						console.log(`${loglv.info}${selflogL} [ToNBR] Terror-Start: `, tonbrRoundData['terrorlist']);
						break;

					case 'TerrorAdded':
						tonbrRoundData['terrorlist'].push(match[1]);

						console.log(`${loglv.info}${selflogL} [ToNBR] Terror-Spawn: ${match[1]}`);
						break;

					default: console.log(`${loglv.info}${selflogL} [ToNBR] ? ${toncontext} ?: ${match.slice(1, tonbrConsts.tonmasks[item].regexIndex + 1)}`);
						break;
				}
				// console.log(`${loglv.info}${selflogL} [ToNBR]`, tonbrRoundData)
				return
			}
		}
	}
...
async function checkAndDeleteWorld(I_worldid, I_targetAmount = 100, I_targetType = 'favorites', I_greaterThan = true) {
	if (!['popularity', 'favorites', 'heat', 'visits'].includes(I_targetType)) { return }
	if (typeof I_greaterThan != 'boolean') { return }
	if (typeof I_targetAmount != 'number') { return }

	var gotWorld = await vrchat.getWorld({ 'path': { 'worldId': I_worldid } })
	if (gotWorld.data != undefined) {

		var conditionCheck = I_greaterThan ? gotWorld.data[I_targetType] >= I_targetAmount : gotWorld.data[I_targetType] < I_targetAmount
		if (gotWorld.data.authorName == '14anthony7095' && conditionCheck) {

			var deletedWorld = await vrchat.deleteWorld({ 'path': { 'worldId': I_worldid } })
			console.log(deletedWorld.data)
	...
	fs.readFile(worldQueueTxt, 'utf8', async (err, fsReadFile) => {
		...
		if (playersInInstance.length >= 2 && currentAccountInUse.id == process.env['VRC_ACC_ID_1'] && InstanceHistory[0].worldHopNoticeSent != true && InstanceHistory[0].groupID == 'grp_c4754b89-80f3-45f6-ac8f-ec9db953adce') {
			...
			if (gotCurrentInstance.data?.queueSize >= 1) {
				playersInQueue = gotCurrentInstance.data.queueSize + Math.ceil(gotCurrentInstance.data.queueSize * 0.2)
			}
		}
		...
		if (created_instance.data != undefined) {
			...
			oscSend(vrcap + `api/explore/next`, false)
			if (fsReadFile.includes(world_id)) {
				fs.writeFile(worldQueueTxt, fsReadFile.replaceAll(`${world_id}\r\n`, ''), (err) => { if (err) { console.log(err) } })
			}
...
async function scanGroupAuditLogs() {
	...
	// Check Worlds to Delete
	// checkAndDeleteWorld('wrld_64b1b32c-aec3-4647-a6a7-d2ad0d4d7eb5', 1, 'heat', false)
...
async function eventHeadingToWorld(logOutputLine) {
	...
	if (groupID == 'grp_c24efb98-3234-4060-94f1-7729523e9689' && worldID == 'wrld_7de12122-be9d-42a1-99c0-4cc763144629') { worldHoppers = { 'worlds': [], 'users': [] } }
	// El Alba starting world
	if (groupID == 'grp_6f6744c5-4ca0-44a4-8a91-1cb4e5d167ad' && worldID == 'wrld_f6445b27-037d-4926-b51f-d79ada716b31') { worldHoppers = { 'worlds': [], 'users': [] } }
	// 14aHop starting world
	if (groupID == 'grp_c4754b89-80f3-45f6-ac8f-ec9db953adce' && worldID == inviteHubQueue(true)) { worldHoppers = { 'worlds': [], 'users': [] } }
				...
				if (I_addToWorldHop) {
					try {
						...
						if (!playerRetention['seenNames'].includes(playerDisplayName)) {
							worldHoppers['users'][whindex]["worldCount"] = (worldHoppers['users'][whindex]["worldCount"] || 0) + 1
						}
					} catch (error) {
						worldHoppers['users'].push({ "name": playerDisplayName, "id": playerID, "playtime": 0, "joinTime": Date.now(), "worldCount": 1, "groupMember": I_memberStatus })
					}
				}
			}
	...
	if (playerDisplayName != undefined || playerDisplayName != null) {
		...
		if (exploreGroups.includes(InstanceHistory[0].groupID)) {
			...
			if (filteredhoppers != undefined) {
				...
				console.debug(worldHoppers['users'][foundindex]["playtime"])
				console.debug(worldHoppers['users'][foundindex]["joinTime"])
				console.debug(Date.now() - worldHoppers['users'][foundindex]["joinTime"])
